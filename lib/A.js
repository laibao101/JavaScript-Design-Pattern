~(function (window) {
    /**
     * 框架单体对象 A
     * @param {str | function} selector 选择器或者页面加载回调函数
    ...
    A.extend({
         ...
         * @return {[type]}      [description]
         */
        fomateString: function (str, data) {
            var html = '';
            //如果渲染数据是数组，则遍历数组并渲染
            if (data instanceof Array) {
                ...
                return str.replace(/\{(\w+)\}/g, function (match, key) {
                    return typeof data === 'string' ? data : (typeof data[
    ...
    A.fn.extend({
        ...
        attr: function () {
            ...
            //如果是一个参数
            if (len == 1) {
                //如果参数是字符串则返回获取到的第一个元素属性值
                if (typeof arg[0] === 'string') {
                    return this[0].getAttribute(arg[0]);

                } else if (typeof arg[0] === 'object') {
                    //如果参数为对象，则为获取到的所有元素设置属性
                    for (i in arg[0]) {
                        if (arg[0].hasOwnProperty(i)) {
                            for (var j = this.length - 1; j >= 0; j--) {
    ...
    /**
     * 避免框架别名冲突（主要用于页面中引入多核框架）
     * @param  {string} library 其他框架
     * @return {[type]}         [description]
     */
